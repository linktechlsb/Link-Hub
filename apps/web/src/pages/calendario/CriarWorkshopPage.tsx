...
import type { CategoriaEvento, Liga, Sala } from "@link-leagues/types";

type CategoriaWorkshop = Extract<CategoriaEvento, "aula" | "cowork" | "encontro">;

interface WorkshopForm {
  liga_id: string;
  categoria: CategoriaWorkshop;
  titulo: string;
  descricao: string;
  data: string;
  sala_id: string;
...
function formVazio(): WorkshopForm {
...
const CATEGORIAS: { value: CategoriaWorkshop; label: string }[] = [
  { value: "encontro", label: "Encontro" },
  { value: "aula", label: "Aula" },
  { value: "cowork", label: "Cowork" },
];

export function CriarWorkshopPage() {
  const navigate = useNavigate();
  const { role, usuarioId } = useUser();
  const [form, setForm] = useState<WorkshopForm>(formVazio);
  ...
  if (sucesso) {
    return (
      <div className="mx-auto max-w-2xl px-8 py-10">
        <div className="mb-8">
          <h1 className="font-display text-2xl font-bold text-foreground">Workshop criado!</h1>
          <p className="mt-1 text-sm text-foreground/50">
            O workshop foi adicionado ao calendário.
          </p>
        </div>
        <button
  ...
  return (
    <div className="mx-auto max-w-2xl px-8 py-10">
      <div className="mb-8">
        <h1 className="font-display text-2xl font-bold text-foreground">Criar Workshop</h1>
        <p className="mt-1 text-sm text-foreground/50">Agende um workshop ou encontro da liga</p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="mt-1">
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, categoria: v as CategoriaWorkshop })}
              >
                <SelectTrigger className={fieldCls}>
                  <SelectValue />
                </SelectTrigger>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Detalhes do workshop (opcional)"
            />
          </div>
          ...
          >
            {salvando ? "Criando..." : "Criar Workshop"}
          </button>
          <button
            onClick={() => navigate("/calendario")}
