  ...
  return (
    <DashboardCard className="dark flex h-[260px] flex-col gap-3 p-5">
      <div className="flex items-center gap-2">
        <h3 className="text-xs text-white/40">Pendências</h3>
        {!loading && itens.length > 0 && (
          <span className="rounded-full bg-brand-yellow/15 px-1.5 py-0.5 text-[10px] font-bold text-brand-yellow">
            {itens.length}
          </span>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-1">
          <CheckSquare className="size-5 text-white/20" />
          <p className="text-center text-xs text-white/40">Tudo em dia por aqui.</p>
        </div>
      ) : (
              ...
              >
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-medium text-white">{item.titulo}</p>
