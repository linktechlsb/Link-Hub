...
const OPCOES_NOTIF: { chave: ChaveNotif; label: string; descricao: string }[] = [
  ...
  { chave: "ranking",  label: "Ranking da liga",             descricao: "Atualizações semanais sobre sua posição no ranking" },
...
function Toggle({ ativo, onToggle }: { ativo: boolean; onToggle: () => void }) {
  return (
    ...
    >
      <span className={cn("pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow-md transform transition-transform", ativo ? "translate-x-5" : "translate-x-0")} />
    </button>
  );
}
...
function AbaNotificacoes({
  ...
  return (
    <div className="space-y-6">
      <div>
        {OPCOES_NOTIF.map(({ chave, label, descricao }) => (
...
export function ContaMembroView() {
  const [abaAtiva, setAbaAtiva] = useState<Aba>("perfil");
  const [dados, setDados] = useState<DadosUsuario>(MOCK_USUARIO);
  const [notif, setNotif] = useState<Notificacoes>({ eventos: true, projetos: true, ranking: false, presenca: true });
