...
function Toast({ mensagem, onFechar }: { mensagem: string; onFechar: () => void }) {
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 bg-foreground text-background text-sm px-4 py-3 shadow-lg rounded">
      {mensagem}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Campo label="Nome completo">
            <input
              ...
              className="w-full px-3 py-2.5 border border-border bg-muted/50 text-sm text-foreground focus:outline-none focus:border-foreground/30 placeholder:text-foreground/20 rounded"
            />
          </Campo>
          <Campo label="E-mail institucional" dica="Somente leitura">
            <input
              ...
              value={dados.email}
              readOnly
              className="w-full px-3 py-2.5 border border-border bg-muted/50 text-sm text-foreground/40 cursor-default focus:outline-none rounded"
            />
          </Campo>
        </div>
